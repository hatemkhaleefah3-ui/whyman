...
async function sendVerification(request: Request, env: Env): Promise<Response> {
  ...
  if (!env.RESEND_API_KEY) return json({error: 'Email service is not configured'}, 503, env, request);
  ...
  if (!response.ok) {
    ...
    return json({error: 'Email could not be sent. Check the Resend sender and API key.'}, 502, env, request);
  }
